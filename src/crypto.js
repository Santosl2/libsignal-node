...
const _infoArrayCache = new WeakMap();

function _getInfoArray(info) {
  if (!_infoArrayCache.has(info)) {
    const arr = new Uint8Array(info.byteLength + 1 + 32);
    arr.set(info, 32); // info nunca muda, pré-preenche aqui
    _infoArrayCache.set(info, arr);
  }
  return _infoArrayCache.get(info);
}
...
function deriveSecrets(input, salt, info, chunks) {
  ...
  const infoArray = _getInfoArray(info);
  ...
  const signed = [
    calculateMAC(
      ...
      Buffer.from(
        infoArray.buffer,
        infoArray.byteOffset + 32,
        infoArray.byteLength - 32,
      ),
    ),
  ];
  if (chunks > 1) {
    ...
    signed.push(
      calculateMAC(
        ...
        Buffer.from(
          infoArray.buffer,
          infoArray.byteOffset,
          infoArray.byteLength,
        ),
      ),
    );
  }
